...
--
-- Temporary table structure for view `commandeslignes`
--

DROP TABLE IF EXISTS `commandeslignes`;
/*!50001 DROP VIEW IF EXISTS `commandeslignes`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `commandeslignes` AS SELECT 
 1 AS `N° Commande`,
 1 AS `Fournisseur`,
 1 AS `Observations`,
...
INSERT INTO `entcom` VALUES (70010,'','2018-04-23 13:59:51',120),(70011,'Commande urgente','2018-04-23 13:59:51',540),(70020,'','2021-10-15 13:59:51',9120),(70025,'Commande urgente','2018-05-23 13:59:51',9150),(70250,'*****','2021-10-20 12:40:50',8700),(70300,'','2018-04-23 13:59:51',9120),(70620,'','2018-04-23 13:59:51',540),(70625,'','2018-06-23 13:59:51',120),(70629,'*****','2021-10-20 12:40:50',9180);
...
--
-- Temporary table structure for view `fournisseurcommandetable`
--

DROP TABLE IF EXISTS `fournisseurcommandetable`;
/*!50001 DROP VIEW IF EXISTS `fournisseurcommandetable`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `fournisseurcommandetable` AS SELECT 
 1 AS `Fournisseur`,
 1 AS `Adresse`,
 1 AS `Code Postal`,
 1 AS `Ville`,
...
INSERT INTO `vente` VALUES ('B001',8700,15,0,50,50,50,100,140),('B002',8700,15,0,210,50,200,100,185),('D035',120,0,0,40,0,40,0,0),('D035',9120,5,0,40,100,30,0,0),('I100',120,90,0,700,50,600,120,500),('I100',540,70,0,710,60,630,100,600),('I100',9120,60,0,800,70,600,90,500),('I100',9150,90,0,650,90,600,200,590),('I100',9180,30,0,876,50,13,100,490),('I105',120,90,10,705,50,630,120,500),('I105',540,70,0,810,60,645,100,600),('I105',8700,30,0,720,50,670,100,510),('I105',9120,60,0,920,70,800,90,700),('I105',9150,90,0,685,90,600,200,590),('I108',120,90,5,795,30,720,100,680),('I108',9120,60,0,920,70,820,100,780),('I110',9120,60,0,950,70,850,90,790),('I110',9180,90,0,973,70,905,90,835),('P220',120,15,0,3700,100,3500,0,0),('P220',8700,20,50,3500,100,3350,0,0),('P230',120,30,0,5200,100,5000,0,0),('P230',8700,60,0,5000,50,4900,0,0),('P240',120,15,0,2200,100,2000,0,0),('P250',120,30,0,1500,100,1400,500,1200),('P250',9120,30,0,1500,100,1400,500,1200),('R080',9120,10,0,120,100,100,0,0),('R132',9120,5,0,275,0,275,0,0);
...
--
-- Final view structure for view `commandeslignes`
--
...
/*!50001 VIEW `commandeslignes` AS select `e`.`numcom` AS `N° Commande`,`f`.`nomfou` AS `Fournisseur`,`e`.`obscom` AS `Observations`,cast(`e`.`datcom` as date) AS `Date de commande`,`p`.`libart` AS `Article`,`l`.`qtecde` AS `Quantité`,`l`.`priuni` AS `Prix unitaire` from (((`entcom` `e` join `ligcom` `l` on((`l`.`numcom` = `e`.`numcom`))) join `fournis` `f` on((`e`.`numfou` = `f`.`numfou`))) join `produit` `p` on((`l`.`codart` = `p`.`codart`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `fournisseurcommandetable`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `fournisseurcommandetable` AS select `f`.`nomfou` AS `Fournisseur`,`f`.`ruefou` AS `Adresse`,`f`.`posfou` AS `Code Postal`,`f`.`vilfou` AS `Ville`,`e`.`numcom` AS `numcom`,`e`.`obscom` AS `obscom`,cast(`e`.`datcom` as date) AS `DATE(e.datcom)` from (`fournis` `f` left join `entcom` `e` on((`f`.`numfou` = `e`.`numfou`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-10-20 17:20:02
