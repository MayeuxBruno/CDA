...
namespace Decouverte
{
    class Program
    {
        ...
        public static char DemanderOperateur()
        {
            char operateur;
            bool isOk;
            do
            {
                Console.Write("Entrez un operateur +,-,*,/,!,v : ");
                isOk = char.TryParse(Console.ReadLine(), out operateur);
            } while (!isOk || (operateur != '+'&& operateur != '-'&&operateur!='*'&&operateur!='/'&&operateur!='$'&&operateur!='!'&& operateur!='v'&&operateur!='='));
            ...
            do
            {
                ...
                if (operateur != '=')
                {
                    switch (operateur)
                    {
                        ...
                        case '+':
                        case '*':
                            valeur2 = DemanderDouble();
                            valeur1 = Calcul(valeur1, valeur2, operateur);
                            break;
                        case '/':
                            valeur2 = DemanderDoubleNonNul();
                            valeur1 = Calcul(valeur1, valeur2, operateur);
                            break;
                        case '$':
                            valeur2 = DemanderEntierPositif();
                            valeur1 = Calcul(valeur1, valeur2, operateur);
                            break;
                        case 'v':
                        case '!':
                            valeur1 = CalculSimple(valeur1, operateur);
                            break;
                    }
